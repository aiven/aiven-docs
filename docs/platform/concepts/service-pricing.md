...
Services are charged by the hour. The minimum hourly charge unit is one hour.
...
by assigning them to a [billing group](/docs/platform/howto/use-billing-groups).
...
Free services do not have any time limitations. However, Aiven reserves the right:
...
- To change the cloud provider, region, or configuration at any point in time.

You can upgrade your free plan to a paid plan at any time by adding
a payment method to the billing group for the service or by starting a free trial.
...
Aiven service plans are optimized for working well in the different clouds
and enable easy migration between the clouds. If the available plans aren't a good fit
for your workload, you can [request a custom plan](/docs/platform/howto/custom-plans)
