...
Aiven MCP groups tools by service. In [read-only mode](#configure-aiven-mcp),
only tools marked **Read-only** below run; write and delete tools are blocked.
If you need to allow a specific write tool while keeping everything else
read-only — for example, allowing topic creation but nothing else — expand
**Allow specific write tools in read-only mode** in the configuration options
above and check the tools to exempt. This adds a `write_allowlist` parameter
to your installation URL.

<details>
<summary>Core (projects, services, logs, metrics)</summary>

| Tool | Read-only | Description |
| --- | --- | --- |
| `aiven_project_list` | Yes | List all Aiven projects. |
| `aiven_project_get` | Yes | Get details for a project. |
| `aiven_list_project_clouds` | Yes | List clouds/regions available for a project. |
| `aiven_service_type_plans` | Yes | List available plans for a service type. |
| `aiven_service_plan_pricing` | Yes | Get pricing for a service plan in a cloud region. |
| `aiven_service_list` | Yes | Search for services across projects. |
| `aiven_service_get` | Yes | Get service details, connectivity, and configuration. |
| `aiven_service_create` | No | Create a new Aiven service. |
| `aiven_service_update` | No | Update an existing service's plan, region, or configuration. |
| `aiven_service_connection_info` | Yes* | Get live connection credentials for a service (requires `allow_secrets=true`; disabled in read-only mode). |
| `aiven_project_get_service_logs` | Yes | Fetch service log entries. |
| `aiven_service_query_activity` | Yes | Fetch current queries and connections for a service. |
| `aiven_service_metrics_fetch` | Yes | Fetch metrics for managed data services. |
| `aiven_service_application_metrics_get` | Yes | Fetch metrics for application services. |
| `aiven_project_vpc_list` | Yes | List VPCs for a project. |
| `aiven_project_get_event_logs` | Yes | Get project event log entries. |
| `aiven_docs_search` | Yes | Search the official Aiven documentation. |
...
| Tool | Read-only | Description |
| --- | --- | --- |
| `aiven_pg_service_available_extensions` | Yes | List extensions available for `CREATE EXTENSION`. |
| `aiven_pg_service_query_statistics` | Yes | Get per-query execution and I/O statistics. |
| `aiven_pg_read` | Yes | Run a read-only SQL query. |
| `aiven_pg_optimize_query` | Yes | Get AI-powered query optimization suggestions (EverSQL). |
| `aiven_pg_write` | No | Run a write SQL statement (DML/DDL). |
| `aiven_pg_bouncer_create` | No | Create a PgBouncer connection pool. |
| `aiven_pg_bouncer_update` | No | Update a PgBouncer connection pool. |
| `aiven_pg_bouncer_delete` | No | Delete a PgBouncer connection pool. |
...
| Tool | Read-only | Description |
| --- | --- | --- |
| `aiven_kafka_topic_list` | Yes | List topics on a Kafka service. |
| `aiven_kafka_topic_get` | Yes | Get details, configuration, and partitions for a topic. |
| `aiven_kafka_topic_message_list` | Yes | Consume messages from a topic. |
| `aiven_kafka_connect_available_connectors` | Yes | List available Kafka Connect connector plugins. |
| `aiven_kafka_connect_list` | Yes | List Kafka Connect connectors. |
| `aiven_kafka_connect_get_connector_status` | Yes | Get runtime status of a connector. |
| `aiven_kafka_schema_registry_subjects` | Yes | List Schema Registry subjects. |
| `aiven_kafka_schema_registry_subject_version_get` | Yes | Get a specific Schema Registry subject version. |
| `aiven_kafka_topic_create` | No | Create a topic. |
| `aiven_kafka_topic_update` | No | Update a topic's configuration. |
| `aiven_kafka_topic_delete` | No | Delete a topic. |
| `aiven_kafka_topic_message_produce` | No | Produce messages to a topic. |
| `aiven_kafka_connect_create_connector` | No | Create a Kafka Connect connector. |
| `aiven_kafka_connect_edit_connector` | No | Edit a connector's configuration. |
| `aiven_kafka_connect_pause_connector` | No | Pause a connector. |
| `aiven_kafka_connect_resume_connector` | No | Resume a connector. |
| `aiven_kafka_connect_restart_connector` | No | Restart a connector. |
| `aiven_kafka_connect_delete_connector` | No | Delete a connector. |
...
| Tool | Read-only | Description |
| --- | --- | --- |
| `aiven_service_integration_list` | Yes | List integrations for a service. |
| `aiven_service_integration_get` | Yes | Get details for a specific integration. |
| `aiven_integration_types_list` | Yes | List available integration types. |
| `aiven_integration_endpoint_types_list` | Yes | List available integration endpoint types. |
| `aiven_service_integration_create` | No | Create a service integration. |
| `aiven_service_integration_update` | No | Update an integration's configuration. |
| `aiven_service_integration_delete` | No | Delete an integration. |
...
| Tool | Read-only | Description |
| --- | --- | --- |
| `aiven_vcs_integration_list` | Yes | List connected VCS (GitHub) accounts. |
| `aiven_vcs_integration_repository_list` | Yes | List repositories accessible via a VCS integration. |
| `aiven_application_deploy` | No | Deploy a Dockerized application. |
| `aiven_application_redeploy` | No | Rebuild and redeploy an existing application after a code push. |

</details>

\* `aiven_service_connection_info` only runs when `allow_secrets=true`, and is
disabled entirely in read-only mode regardless of the write allowlist — see
[Security and responsibility](#security-and-responsibility).
