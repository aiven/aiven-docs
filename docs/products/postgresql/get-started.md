...
1.  `cd` to the `dellstore2-normal-1.0` folder using your terminal.

1.  [Connect to your PostgreSQL instance with `psql`](/docs/products/postgresql/howto/connect-psql).

1.  Create a `dellstore` database and connect to it:
...
Retrieve all data from a table, for example, from `orders`:
