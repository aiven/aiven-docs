...
Use the <ConsoleIcon name="sql editor"/> to write, edit, and execute queries.

:::note
The <ConsoleIcon name="sql editor"/> is on by default. If it is off for your organization,
it shows a message to contact Aiven support and you cannot open the editor or run
queries. Aiven manages this control. To change it, contact the Aiven support team. See
...
1. In the <ConsoleIcon name="sql editor"/>, enter your query.
...
1. In the <ConsoleIcon name="sql editor"/>, place your cursor inside the query you want
   to run, without selecting any text.
1. Click **Run**, or press `Cmd+Enter` on macOS or `Ctrl+Enter` on Windows and Linux.
1. View the results in the results panel.
...
statements you want to run. The <ConsoleIcon name="sql editor"/> runs them sequentially
and displays the results in separate tabs.

1. In the <ConsoleIcon name="sql editor"/>, write your SQL statements:
...
- The <ConsoleIcon name="sql editor"/> automatically detects the query type and applies
  appropriate safeguards.
- The editor executes it unless it is a write operation, in which case it prompts
...
1. In the <ConsoleIcon name="sql editor"/>, enter your write query.
1. Click **Run**, or press `Cmd+Enter` on macOS or `Ctrl+Enter` on Windows and Linux.
1. The <ConsoleIcon name="sql editor"/> detects the write operation and shows **You are
   changing live data**.
...
The <ConsoleIcon name="sql editor"/> keeps a change log of write queries you run. You can
review changes and create a fork from a point in time before a specific write query.

To view the change log:

1. In the <ConsoleIcon name="sql editor"/>, click **Change log**. This option appears
   after you run at least one write query.
...
1. Click **Create fork**. The <ConsoleIcon name="sql editor"/> opens **Create fork** with
   a recovery timestamp set to the moment before that query ran.

## Query execution limits

The <ConsoleIcon name="sql editor"/> applies these limits to query execution:
