...
[SSL](/docs/products/valkey/howto/manage-ssl-connectivity), encrypts the connection
between a client and the service. Authorization, through [access control lists
(ACLs)](/docs/products/valkey/howto/configure-acl-permissions), determines what an
...
- Every service starts with a default administrative user that has full access.
  Create separate [service users](/docs/products/valkey/howto/manage-service-users)
  with restricted ACLs for applications and integrations instead of sharing that
  user's credentials.
- Direct `ACL` and [`CONFIG`](/docs/products/valkey/reference/restricted-commands)
  commands are restricted on the server. Configure ACLs and SSL settings through the
  Aiven Console, Aiven CLI, Aiven API, or Terraform instead.
