...
## Schema Registry (Karapace) metrics

When [Schema Registry](/docs/products/kafka/karapace/howto/enable-karapace) is enabled for
an Aiven for Apache Kafka® service, metrics from the underlying Karapace Schema Registry
instance are exposed through the Prometheus metrics endpoint. Use the same
...
| Metric                                | Description                                                                 |
|---------------------------------------|-----------------------------------------------------------------------------|
| `karapace_http_requests_total`        | Total HTTP requests. Use `rate()` for requests per second.                 |
| `karapace_http_requests_duration_seconds` | Request duration. Prometheus exposes `_sum` and `_count`; use both for average duration in seconds. |
| `karapace_http_requests_in_progress`  | HTTP requests currently in progress.                                       |
