...
Enabling cross-cluster replication (CCR) requires [building a CCR setup on the Aiven platform](#enable-ccr-in-aiven) and, next, [configuring the replication on the Apache Cassandra side](/docs/products/cassandra/howto/manage-cross-cluster-replication#set-up-replication-factor).

## About enabling CCR

You can enable CCR either when creating a new Aiven for Apache Cassandra®
service or for an existing service. You can achieve that in the Aiven Console, using Aiven
CLI, or programmatically with Aiven API.
...
-   Check
    [CCR limitations](/docs/products/cassandra/concepts/cross-cluster-replication#ccr-limitations).
...
-   New Aiven for Apache Cassandra service by
    [creating a totally new CCR service pair](/docs/products/cassandra/howto/enable-cross-cluster-replication#new-pair)
    or
...
1.  From the **Services** page, select **+Create service**.
1.  In the **Create service** view
    1.  Select Apache Cassandra® and, next, a cloud provider and a cloud
        region for your service.
    1.  To activate CCR, select the **Enable Cross Cluster Replication**
        toggle, which unfolds an additional section for setting up
        another Apache Cassandra service.
    1.  Select a cloud provider and a cloud region for the second
        service.
    1.  Select one service plan for your two new services.
...
1.  In the **Overview** page of your service, navigate to the **Cross
...
1.  In the **Create a Cross Cluster Replica** view

    1.  Select a cloud provider and a cloud region, define a name for
        your new service, and select **Continue**.
    1.  Examine the **Service Summary** section and make sure the
...
1.  Create another new service (`service_2`). This time, include the
    ...
    :::important
    See
    [Limitations](/docs/products/cassandra/concepts/cross-cluster-replication#ccr-limitations)
...
Use the [avn service create](/docs/tools/cli/service-cli#avn-cli-service-create) command
to create a new service with CCR enabled. Use the
...
1.  Create another new service (`service_2`). This time when
