...
handlebars.registerHelper('parameterDetailsHelper', function (options) {
  ...
  if (minimum || maximum || def || restart_warning) {
    ...
    if (restart_warning) constraints.push('<li><code>warning: causes a restart</code></li>');
    ...
    <tr>
      <td>
        ...
        <table className="service-param-children">
          ...
          <tr>
            <td>
              {{parameterDetailsHelper name=@key parent=@../key type=type minimum=minimum maximum=maximum def=default restart_warning=x-change-requires-restart}}
              {{#if title~}}<p className="title">{{title}}</p>{{~/if}}
              {{#if description~}}<div className="description"><p>{{description}}</p></div>{{~/if}}
            </td>
          </tr>
